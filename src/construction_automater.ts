...
// TODO: Construct roads between drop point / storage and room controller
  ...
  public setupSources() {
    ...
    if (this.room.memory.sources.length === 0) {
      ...
      for (const source of sources) {
        ...
        const container: Structure | undefined = (source.pos.findInRange<StructureContainer>(FIND_STRUCTURES, 1, { filter: (structure) => structure.structureType === STRUCTURE_CONTAINER })[0] ||
                                                  source.pos.findInRange(FIND_MY_CONSTRUCTION_SITES, 1, { filter: (c) => c.structureType === STRUCTURE_CONTAINER })[0]);

        if (container) {
          pathSteps = position.findPathTo(container.pos, { ignoreCreeps: true });
        ...
        // Construct container
        pathSteps = position.findPathTo(source.pos, { ignoreCreeps: true });
